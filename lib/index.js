...
var createWatcher = function createWatcher(_ref) {
    ...
    var watcher = chokidar.watch(store.target, {
        persistent: true,
        ignored: /(^|[\/\\])\..*(?<!\.js)$/, //忽略点文件
        cwd: '.', //表示当前目录
        depth: 99 //到位了....
...
var bindMockServer = function bindMockServer(app) {
    ...
    app.use(dmStart);
    // 添加路由
    Object.entries(requireFile(glob.sync(db + '/!(.)*.js'))).forEach(function (_ref) {
    ...
    var indexArr = app._router.stack.reduce(function (r, _ref3, index) {
        var name = _ref3.name;
...
var bindServer = function bindServer(_ref4) {
    var server = _ref4.server,
        _ref4$target = _ref4.target,
        target = _ref4$target === undefined ? store.target : _ref4$target;
